...
在 MATLAB 的命令窗口中按照下述方法进行操作，就可以了解该控制系统的幅频特性和相频特性。在 MATLAB 的命令窗口中键入以下语句：

```matlab
num=[-11, 0]; den=[1, -12, 1, -1];
bode(num, den);       %命令函数 bode() 用于获得控制系统的幅频特性和相频特性
grid on               %命令 grid on 或命令 grid off 分别表示添加和删除栅格线
```
